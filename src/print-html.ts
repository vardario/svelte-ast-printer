...
class ElementPrinter extends BaseHtmlNodePrinter {
  attributeValueToString(attribute: Attribute | SpreadAttribute) {
    if (attribute.value) {
    ...
    if (attribute.expression) {
      return `{${generate(attribute.expression)}}`;
    }
      ...
      elementNode.attributes
        ?.map(attribute => {
          ...
          if (value && value.type === 'AttributeShorthand') {
            return this.attributeValueToString(attribute as Attribute);
          }
          ...
          return `${attributeName}=${this.attributeValueToString(attribute as Attribute)}`;
        })
        .join(' ') ?? '';

    if (elementNode.expression) {
      write(` this={${generate(elementNode.expression)}}`);
    }

    if (elementNode.tag) {
      write(` this={${generate(elementNode.tag)}}`);
    }
...
class MustacheTagPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    ...
    context.write(`{${generate(mustacheTagNode.expression)}}`);
...
class EachBlockPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    ...
    write(`{#each ${generate(node.expression)}`);

    if (node.context) {
      write(` as ${generate(node.context)}`);
    }
...
class AwaitBlockPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write(`{#await ${generate(node.expression)}}`);
  }
  leave(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write(`{/await}`);
...
class KeyBlockPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write(`{#key ${generate(node.expression)}}`);
  }
  leave(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write('{/key}');
...
class RawMustacheTagPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write(`{@html ${generate(node.expression)}}`);
    context._this.replace({
      ...node,
      expression: undefined
    });
...
class DebugTagPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write(`{@debug ${node.identifiers.map((id: any) => generate(id)).join(', ')}}`);
  }
  leave(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
  }
...
class ConstTagPrinter extends BaseHtmlNodePrinter {
  enter(node: TemplateNode, parent: TemplateNode, context: PrinterContext) {
    const { write } = context;
    write(`{@const ${generate(node.expression)}}`);
    context._this.replace({
      ...node,
      expression: undefined
    });
